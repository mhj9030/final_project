...
<script type="text/javascript">
  ...
  function check() {
       ...
       /*  str = f.upload.value;
        ...
        var mode="${mode}";
        if(mode=="created")
    		f.action="<%=cp%>/community/photo/created";
    	else if(mode=="update")
    		f.action="<%=cp%>/community/photo/update";
							...
							<tr height="40" style="border-bottom: 1px solid #cccccc;">
						        ...
						        <td style="padding-left:10px;">
						            ...
						            <c:if test="${not empty dto.originalFilename}">
						            	| <a href="<%=cp%>/community/photo/deleteFile?ptnum=${dto.ptnum}&page=${page}">삭제</a>
						            </c:if>
						        </td>
						    </tr>
						</c:if>
					...
					<tfoot>
						<tr>
							<td colspan="4" style="text-align: center;">
								...
								<button type="button" class="btn btn-default" onclick="javascript:location.href='<%=cp%>/community/photo';">${mode=='update'?'수정취소':'등록취소'}</button> 
								 <c:if test="${mode=='update'}">
						         	 <input type="hidden" name="ptnum" value="${dto.ptnum}">
