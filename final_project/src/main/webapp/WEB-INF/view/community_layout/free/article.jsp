...
<script type="text/javascript">
	function deleteFree() {
		  var frnum = "${dto.frnum}";
		  var page = "${page}";
		  var query = "frnum="+frnum+"&page="+page;
		  var url = "<%=cp%>/community/free/delete?" + query;
	...
	function updateFree() {
		  ...
		  var page = "${page}";
		  var query = "frnum="+frnum+"&page="+page;
		  var url = "<%=cp%>/community/free/update?" + query;
		  location.href=url;
...
function countLikeFree(frnum) {
	...
	$.ajax({
		...
		,success:function(data) {
			var count=data.countLikeFree;
			
			$("#countLikeFree").html(count);
		}
...
function sendLikeFree(frnum) {
	...
	$.ajax({
		type:"post"
		,url:"<%=cp%>/community/free/insertLikeFree"
		,data:query
		,dataType:"json"
		,success:function(data) {
...
<script type="text/javascript">
	...
	function listPage(page){
		var url = "<%=cp%>/community/free/listReply";
		var frnum = "${dto.frnum}";
		
		// text/html
		$.post(url, {frnum:frnum, pageNo:page}, function(data){
			$("#listReply").html(data);
		});
	}
	...
	function sendReply(){
		...
		$.ajax({
			type:"post"
			,url:"<%=cp%>/community/free/insertReply"
			,data:query
			,dataType:"json"
			,success:function(data) {
				var state = data.state;
	...
	function listAnswer(answer) {
		...
		var url="<%=cp%>/community/free/listReplyAnswer";
	...
	function deleteReply(replyNum, page) {
		...
		if(confirm("게시물을 삭제하시겠습니까 ? ")) {	
			var url="<%=cp%>/community/free/deleteReply";
	...
	function countAnswer(answer) {
		var url="<%=cp%>/community/free/replyCountAnswer";
...
<div class="community_article_wrap">
		...
		<tr>
			...
			<td class="article_next">
				<c:if test="${not empty nextReadDto}">
			    	<a href="<%=cp%>/community/free/article?${query}&frnum=${nextReadDto.frnum}">${nextReadDto.subject}</a>
			    </c:if>
				 다음글▶  
			</td>
		</tr>
	...
	<div class="article_btns">
		...
		<button class="btn btn-default" onclick="javascript:location.href='<%=cp%>/community/free?${query}'">목록</button>
		<button class="btn btn-default" onclick="javascript:location.href='<%=cp%>/community/free'">최신목록</button>
	</div>
